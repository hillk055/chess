...
class Chess_Game:
    ...
    def Player_Turn(self, row, col):
        ...
        if find_piece == '_':
            return []
        has_moved = self.moved[find_piece] if find_piece in self.moved.keys()
        color = find_piece[0]
    ...
    def update_board(self, current_xy, new_xy):
        ...
        if new_xy in moves_to_make:
            ...
            return looking_for_piece, new_xy
...
class Pieces:
    ...
    def pawn(self) -> list:
        ...
        if self.color == 'b':
            move_forward = 1
            first_move = 2
            if self.board[self.row + 1][self.column + 1] != '_':
        ...
        print(remove_possible_squares)
        print(f'possible{self.possible_moves}')
                        ...
                        def king(self) -> list:
                            ...
                            for dx in king_moves:
                                for dy in king_moves:
                                    ...
                                    if 0 <= square[0][0] <= self.BOARD_SIZE - 1 and 0 <= square[0][
                                        1] <= self.BOARD_SIZE - 1:
